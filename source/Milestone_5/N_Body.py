...
def F_NCuerpos(U, t, Nb, Nc): 
     ...
     # Reshape de la solución y las derivadas a una forma tridimensional     
     Us  = reshape( U, (Nb, Nc, 2) )  
     F =  zeros(len(U))   
     dUs = reshape( F, (Nb, Nc, 2) )  
     
     # Obtener las posiciones y velocidades de la solución
...
def Resolver_NCuerpos():  
    ...
    # Definir la función F para el problema de N cuerpos
    def F(U, t): 
        # Llama a la función que calcula la F
        return F_NCuerpos(U, t, Nb, Nc)  

    N = 1000
    Nb = 4      # Número de cuerpos
    Nc = 3      # Número de coordenadas
    Nt = (N + 1) * 2 * Nc * Nb  # Dimensión del vector de estado
    ...
    # Cuerpo 1
    r0[0, :] = [1, 0, 0]        # Posición inicial del cuerpo 1
    v0[0, :] = [0, 0.4, 0]      # Velocidad inicial del cuerpo 1

    # Cuerpo 2
    r0[1, :] = [-1, 0, 0]       # Posición inicial del cuerpo 2
    v0[1, :] = [0, -0.4, 0]     # Velocidad inicial del cuerpo 2

    # Cuerpo 3
    r0[2, :] = [0, 1, 0]        # Posición inicial del cuerpo 3
    v0[2, :] = [-0.4, 0., 0.]   # Velocidad inicial del cuerpo 3

    # Cuerpo 4
    r0[3, :] = [0, -1, 0]       # Posición inicial del cuerpo 4
